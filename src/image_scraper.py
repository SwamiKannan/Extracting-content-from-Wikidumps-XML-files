...
def parse_image_data(text, url):
    while "[[File:" in text:
        count = 0
        for r in range(len(text) - 1):
            st_index = text.find('[[File:') + 7
            if text[r:r + 2] == '[[':
                count += 1
            if text[r:r + 2] == ']]':
                count -= 1
                if count == 0:
                    text = text[st_index:r]
...
def get_images(input_files, output_folder='images'):
    if output_folder not in os.listdir():
        os.makedirs(output_folder)
    out_path = os.path.join(output_folder, name)
    print('Output path', out_path)
    image_links = [(a['filename'], a['image_url']) for a in input_files]
    for (name, url) in image_links:
        ...
        if response.status_code == 200:
            ...
            urllib.request.urlretrieve(url_img, out_path)
            print(f' {name} file written')
        else:
            print(url)


def process_images(text, url):
    file_lists = parser(text, url)
    get_images(file_lists, 'images')


test_link = "https://en.wikipedia.org/wiki/Overview_of_gun_laws_by_nation"
# print(list(map(parse_image_data, check_text)))
files = parse_image_data(sample_text, test_link)
for file in files:
